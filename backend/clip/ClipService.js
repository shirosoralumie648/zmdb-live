...
export default class ClipService {
    ...
    insert = async (ctx) => {
        ...
        if (bv) {
            ...
            if (entity.type === 2 || entity.type === 3) {
                ...
                clip.playUrl = `${config.zimu.local.url}/${author.organizationId}/${author.name}/${datetime.substring(0, 7)}/${datetime.replaceAll('-', '').replaceAll(':', '').replaceAll(' ', '-')}-${author.name}-${title}.mp4`;
                clip.redirectUrl = '';
            } else if (entity.type === 4) {
                // TODO
                clip.type = entity.type;
                clip.playUrl = `${config.zimu.live.url}/${author.organizationId}/${author.name}/${datetime.substring(0, 7)}/${datetime.replaceAll('-', '').replaceAll(':', '').replaceAll(' ', '-')}-${author.name}-${title}.flv`;
                clip.redirectUrl = '';
            } else {
                clip.type = 0; // 未知来源
                clip.playUrl = '';
    ...
    update = async (ctx) => {
        ...
        if (entity.hasOwnProperty('type')) {
            ...
            if (clip.type === 2 || clip.type === 3) {
                clip.playUrl = `${config.zimu.local.url}/${author.organizationId}/${author.name}/${clip.datetime.substring(0, 7)}/${clip.datetime.replaceAll('-', '').replaceAll(':', '').replaceAll(' ', '-')}-${author.name}-${clip.title}.mp4`;
                clip.redirectUrl = '';
            } else if (clip.type === 4) {
                clip.playUrl = `${config.zimu.live.url}/${author.organizationId}/${author.name}/${clip.datetime.substring(0, 7)}/${clip.datetime.replaceAll('-', '').replaceAll(':', '').replaceAll(' ', '-')}-${author.name}-${clip.title}.flv`;
                clip.redirectUrl = '';
            }
        }
    ...
    __parseUrl = (clip) => {
        if (clip.type === 3) {
            clip.playUrl = `${config.zimu.local.url}/${clip.author.organizationId}/${clip.author.name}/${clip.datetime.substring(0, 7)}/${clip.datetime.replaceAll('-', '').replaceAll(':', '').replaceAll(' ', '-')}-${clip.author.name}-${clip.title}.mp4`;
            clip.redirectUrl = '';
        } else if (clip.type === 4 || clip.type === 5) {
            clip.playUrl = `${config.zimu.live.url}/${clip.author.organizationId}/${clip.author.name}/${clip.datetime.substring(0, 7)}/${clip.datetime.replaceAll('-', '').replaceAll(':', '').replaceAll(' ', '-')}-${clip.author.name}-${clip.title}.flv`;
            clip.redirectUrl = '';
        }
        return clip;
    }
