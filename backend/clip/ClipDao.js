export default class ClipDao {
    ...
    findByBv = (bv) => {
        const sql = 'SELECT ' + 
                    ...
                    'LEFT JOIN author ON clip.authorId=author.id ' +
                    'LEFT JOIN organization ON author.organizationId=organization.id ' +
                    'WHERE clip.playUrl like ?';
        const stmt = this.db.prepare(sql);
        return stmt.all(`%bvid=${bv}%`).map(item => this.__toDTO(item));
    }
    ...
    findByType = (type) => {
        ...
        const stmt = this.db.prepare(sql);
        return stmt.all(type).map(item => this.__toDTO(item));
    }
    ...
    findByAuthorId = (authorId, type, page, size) => {
        const sql = 'SELECT ' + 
                    ...
                    'LEFT JOIN author ON clip.authorId=author.id ' +
                    'LEFT JOIN organization ON author.organizationId=organization.id ' +
                    'WHERE clip.authorId=? AND type=? ' +
                    'ORDER BY clip.id DESC LIMIT ?,?';
        const stmt = this.db.prepare(sql);
        return stmt.all(authorId, type, (page - 1) * size, size).map(item => this.__toDTO(item));
    }
    ...
    findByOrganizationId = (organizationId) => {
        const sql = 'SELECT ' + 
                    ...
                    'ORDER BY clip.datetime DESC, clip.playUrl DESC';
        const stmt = this.db.prepare(sql);
