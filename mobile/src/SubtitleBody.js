...
export const SubtitleBody = () => {
    ...
    const checkAudio = Form.useWatch("audio", segmentForm);
    const checkDanmaku = Form.useWatch("danmaku", segmentForm);
    ...
    const onSegment = async (values) => {
        ...
        const danmaku = values.danmaku;
        const forward = values.forward || 0;
        ...
        try {
            const json = await ClipApi.fetchSegment(selectedClip.id, startTime, endTime, audio, danmaku, forward);
            const filename = json.filename;
            if (!json.filename || json.filename === '') {
                throw {code: 20000, message: '切片失败'};
            }
            let url = `${config.static.url}/segments/${filename}`;
    ...
    return (
        <div style={{ position:'relative', width: '100%', height:'100%', display:'flex', flexDirection:'column' }}>
            {
                selectedClip.type === 1 ? 
                    <iframe title={selectedClip.title} width="100%" height="240px" src={`//${selectedClip.playUrl}&t=${currentTime}`} scrolling="no" border="0" frameBorder="no" framespacing="0" allowFullScreen={true}></iframe>
                :
                    <ArtPlayer
                        option={{
                            url: `${selectedClip.playUrl}`,
                            poster: `${selectedClip.cover}`,
                            subtitle: {
                            ...
                            danmakuUrl: selectedClip.type === 2 ? `${selectedClip.playUrl.replaceAll('.mp4', '.xml').replaceAll('/d/', '/p/')}` : `${selectedClip.playUrl.replaceAll('.mp4', '.xml').replaceAll('.flv', '.xml')}`
                        }}
                        style={{
                            width: '100%',
                            height: '240px',
            ...
            <div style={{ flex:" 0 0 auto", width:"100%", height:"39px", display:'flex', justifyContent:'space-between' }}>
                ...
                >
                    ...
                    >
                        ...
                        <Form.Item
                            disabled={selectedClip.type === 1 || checkAudio}
                            name='danmaku'
                            label='弹幕'
                            childElementPosition='right'
                        >
                        ...
                        <Form.Item
                            ...
                            label='弹幕提前（秒）'
                        >
                            <Input placeholder='请输入弹幕提前时间，默认为0' />
                        </Form.Item>
                        {/* <Form.Item
                            name='filename'
                            label='切片文件名'
